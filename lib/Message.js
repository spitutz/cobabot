...
async forwardMessage(targetJid, message, options = {}) {
    let contentType;
    let content = message.quoted.msg;  // Safe access to message.message

    if (options.readViewOnce) {
        content = content?.ephemeralMessage?.message || content || undefined;

        if (content?.viewOnceMessage?.message) {
            const viewOnceKey = Object.keys(content.viewOnceMessage.message)[0];
            delete (content?.ignore ? content.ignore : content);
            delete content.viewOnceMessage.message[viewOnceKey].viewOnce;
            content = { ...content.viewOnceMessage.message };
        }
    }

    if (options.mentions && contentType) {
        content[contentType].contextInfo = content[contentType].contextInfo || {};
    ...
    if (options.mentions) {
        forwardContent[contentType].contextInfo = forwardContent[contentType].contextInfo || {};
        forwardContent[contentType].contextInfo.mentionedJid = options.mentions;
    }

    let contextInfo = {};
    if (contentType !== "conversation" && message?.message?.[contentType]?.contextInfo) {
